...
public class CacheLRU implements Cache {
    ...
    CacheLRU(CacheWriter writer, int maxMemory) {
        this.writer = writer;
        this.maxMemory = maxMemory;
        this.memory = 0;
        this.len = maxMemory/64;
        this.mask = len-1;
        clear();
    }

    public static Cache getCache(CacheWriter writer, int cacheSize) {
        return new CacheLRU(writer, cacheSize);
    }
    ...
    public void put(CacheObject rec) {
        ...
        if (old != null) {
            System.err.println("ERROR: try to add a record twice at pos " + pos);
        }
    ...
    public CacheObject update(int pos, CacheObject obj) {
        ...
        if (old == null) {
            ...
            if (old != obj) {
                System.err.println("ERROR: old!=record pos:" + pos + " old:" + old + " new:" + obj);
            }
            removeFromLinkedList(obj);
            addToFront(obj);
        }
